...
def gap_fill_algorithm(data,relation_dict,is_bad_observation,gf_key):
    ...
    for year in sorted(data.keys()):
        ...
        print([str(str(day)) for day in sorted([int(day.encode('UTF8')) for day in list(data[year].keys())])])
        for day in [str(str(day)) for day in sorted([int(day.encode('UTF8')) for day in list(data[year].keys())])]:
            print(("\t"+"day:\t"+str(day)+"->"))
