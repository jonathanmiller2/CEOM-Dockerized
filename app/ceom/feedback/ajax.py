from ceom.feedback.models import Task_status, Feedback, Comment
import json
...
def updateComment(request,feed_id,new_val):
	...
	x.save()
	send_mail('New Comment added!', 'Form_id : '+str(x.Comment_id.id)+'\n'+'Details: '+'\n'+'subject:'+str(x.Comment_id.subject)+'\n'+'Comment : '+str(x.Comment_text)+'\n'+'Link : '+'ceom.ou.edu/feedback/comment/'+str(x.Comment_id.id)+'\n', 'admin@ceom.ou.edu', ['bhargavreddy.bolla@ou.edu','bhargavreddy.bolla@gmail.com','Xibei.Jia-1@ou.edu'], fail_silently=True)
	return json.dumps({'message':"Success"})
